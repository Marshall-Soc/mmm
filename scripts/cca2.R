...
# NOTE: Adapted from https://github.com/cran/corclass/blob/master/R/cca.R
...
cca2 <- function (dtf, filter.significance = TRUE, 
  ...
  if (filter.significance == TRUE) {
    ...
    print("Not filtering significances.\n")
  }
  
  graph <- .cormat.to.igraph (cormat, absolute.value = TRUE)
  comm <- igraph::leading.eigenvector.community(graph, steps=6)
